...
public class PythonCheckerCore extends CheckerCaller {

    String root = System.getenv("BJUT_OJ_HOME");

    String debugInfo;
    public PythonCheckerCore(String submissionUUID, String debugInfo, String targetProblem) {
        super();
        String fname = "main" + submissionUUID + ".py";
        if (root != null) {
            root += "\\files";
        } else {
            throw new RuntimeException("Cannot locate file storage");
        }
        setChecker(new PythonChecker(root, submissionUUID, targetProblem));
